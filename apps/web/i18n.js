...
export function readLocaleCookie(cookieHeader) {
  for (const cookieName of [LOCALE_COOKIE_NAME, ...LEGACY_LOCALE_COOKIE_NAMES]) {
    for (const entry of parseCookiePairs(cookieHeader)) {
      ...
      return normalizeDashboardLocale(rest.join('=')) ?? null
    }
  }
...
export function writeLocaleCookie(doc, locale) {
  if (!doc) {
    return
  }

  const normalizedLocale = normalizeDashboardLocale(locale) ?? DEFAULT_LOCALE
  doc.cookie = `${LOCALE_COOKIE_NAME}=${normalizedLocale}; Path=/; Max-Age=31536000; SameSite=Lax`
}
