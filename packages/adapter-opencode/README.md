...
- allow an opt-in wrapper-only `CLIPULSE_OPENCODE_ENABLE_SESSION_DIFF=1` path that strips `session.diff` down to `{ path, additions, deletions }`, drops repo-external or `../`-escaping paths before bridge output, and drops paths already seen via `file.edited` in the same buffered phase
