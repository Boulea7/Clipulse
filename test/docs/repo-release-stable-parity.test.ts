...
const ENGLISH_README = new URL('../../README.en.md', import.meta.url)
...
describe('repo release stable parity', () => {
  ...
  it('keeps the tagged release workflow pinned to the requested tag and backed by the shared release asset manifest', () => {
    ...
    expect(workflow).toContain('release_lookup_status=0')
    expect(workflow).toContain('jq -r --arg tag "${RELEASE_TAG}"')
    expect(workflow).toContain('select(.tag_name == $tag)')
    expect(workflow).toContain('Failed to list GitHub releases for ${RELEASE_TAG}.')
    expect(workflow).toContain('matching_release_count=')
    expect(workflow).toContain('matching_draft_count=')
    expect(workflow).toContain('matching_non_draft_count=')
    expect(workflow).toContain('.created_at')
    expect(workflow).toContain('.html_url')
    expect(workflow).toContain('Found duplicate draft releases for ${RELEASE_TAG}')
    expect(workflow).toContain('Delete stale drafts by release id or in the GitHub UI')
    ...
    expect(workflow).not.toContain('gh release delete "${RELEASE_TAG}" --yes')
  ...
  it('keeps the packaged dashboard contract and public community docs aligned to the stable release surface', () => {
    ...
    assertContains(CHANGELOG, changelog, '0.1.1')
  ...
  it('keeps release-facing docs aligned on Python artifact smoke scope and stable asset families', () => {
    ...
    const englishReadme = expectFile(ENGLISH_README)
    ...
    assertContains(ENGLISH_README, englishReadme, 'npm run check:py-build')
    assertContains(ENGLISH_README, englishReadme, 'node scripts/release-assets.mjs manifest')
    assertContains(ENGLISH_README, englishReadme, 'node scripts/release-assets.mjs checksums')
    assertContains(ENGLISH_README, englishReadme, 'npm run check:release-assets:stable')
    ...
    assertContains(RELEASE_AND_PACKAGING, releaseGuide, 'duplicate drafts for the same tag')
    assertContains(RELEASE_AND_PACKAGING, releaseGuide, 'delete stale drafts by release id')
    assertContains(RELEASE_AND_PACKAGING, releaseGuide, 'do not use tag-level `gh release delete <tag>` or `--cleanup-tag`')
