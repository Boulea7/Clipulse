...
- `packages/adapter-opencode` 目前仍只把顯式 `file.edited` 視為高置信 delta 來源；若官方 `file.edited` 只提供路徑，Clipulse 也只會先記錄 path-only delta，不抓 transcript、不接 server API，也不直接吞整條 message/TUI event 流。wrapper 會先按宣告的 project root 過濾明顯越界的路徑，而 bridge 仍會依 `cwd` 解析到的最近 git root 做最終專案歸屬，因此目前的公開保證仍應理解為「先擋掉明顯的 project 外路徑」，而不是「所有 wrapper / bridge 場景都嚴格共用同一個 root」。
