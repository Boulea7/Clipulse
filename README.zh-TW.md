...
- 後續只有在官方 lifecycle contract 與預設 wiring 路徑都足夠穩定、能持續產出高置信 file delta 時，`Gemini CLI`、`OpenCode` 才會再推進到一等穩定支援
...
先跑 `npm run smoke:stable` 作為穩定面 gate；如果要檢查實驗接入，再單獨跑 `npm run smoke:experimental`。下面這些命令只用於診斷，不替代 smoke gate；如果需要更完整的 operator 流程、狀態目錄細節與回應示例，再繼續看 [docs/self-hosting-and-integration.md](./docs/self-hosting-and-integration.md)：
...
- 更完整的轉接器邊界說明見 `packages/adapter-codex/README.md`；目前 checked-in 的 canonical wiring source 是 `packages/adapter-codex/examples/hooks.json`。若宿主提供 `PostToolUseFailure` / `StopFailure` / `SessionEnd`，建議保持接線，它們能幫助 Clipulse 更完整地結算 wait 和清理本機狀態
...
- Gemini 的第一真相來源刻意收斂在 `packages/adapter-gemini/README.md` 與 checked-in wiring 範例 `packages/adapter-gemini/examples/.gemini/settings.json`；頂層 setup 文件只保留 operator 摘要，不再維護第二份 hook contract 真相。
- `BeforeAgent` 與相容 alias `UserPromptSubmit` 不應在同一套接線裡同時保留；若官方 `BeforeAgent` 可用，應優先保留 `BeforeAgent` / `AfterAgent` 這組主路徑。
- `packages/adapter-opencode/dist/plugin.js` 目前仍是一個薄的 bridge 入口，而不是可直接落地的完整 plugin；OpenCode 的第一真相來源刻意收斂在 `packages/adapter-opencode/README.md` 與 checked-in wrapper 範例 `packages/adapter-opencode/examples/clipulse.ts`，頂層文件同樣不再維護第二份 wrapper contract 真相。
- `packages/adapter-opencode` 目前仍以顯式 `file.edited` 作為預設高置信 delta 來源；`session.diff` 繼續保持 default-off，只有明確設定 `CLIPULSE_OPENCODE_ENABLE_SESSION_DIFF=1` 時，checked-in wrapper 範例才會做 wrapper-only backfill，而且仍只會轉發最小 `{ path, additions, deletions }`。
- 這兩個接入面目前都維持「可試接入但仍實驗性」：已可建置、可跑 fixture / contract test，也已有最小自託管 wiring 說明，但提升為一等支援仍取決於官方 lifecycle contract 穩定、預設 wiring 路徑能提供高置信 file delta，以及 checked-in canonical wiring 範例和 fixture/contract coverage 能持續覆蓋成功/失敗清理路徑。
...
探針分工、`/api/v1/status` 的機器可讀預期，以及包含首次空態在內的運行時回應示例，統一下沉到 [docs/self-hosting-and-integration.md](./docs/self-hosting-and-integration.md) 為準，README 這裡不再重複展開完整 operator 契約。
...
把這些 badge SVG 路由視為公開圖片 surface；當你想讓 Clipulse 回傳 canonical 的 README Markdown 片段時，再使用 `/api/v1/public/readme/*`。
