...
[简体中文](./README.md) | [English](./README.en.md) | [日本語](./README.ja.md) | [Español](./README.es.md) | [Français](./README.fr.md) | [한국어](./README.ko.md)
...
> 面向 `Claude Code`、`Codex`、`Gemini CLI` 與 `OpenCode` 的隱私優先、自託管活動追蹤。

Clipulse 是面向 coding-agent CLI 的自託管活動追蹤器。它會把本地 hooks 和 plugin 事件整理成隱私友好的彙總、輕量 dashboard，以及可嵌入 README 的 badge，預設不上傳原始碼正文和 raw prompt。

## 為什麼是 Clipulse

- API、SQLite 和 dashboard 都留在你自己的基礎設施裡。
- 用一套受限事件契約追蹤 active time、wait time、file delta、語言、模型和 host mix。
- 需要公開展示時，只放 badge 和 README snippet，不必公開私有 dashboard。
- 現在可以先走原始碼 checkout，後續再切到更乾淨的 Python release artifact 部署路徑。

預設傳輸會保留彙總所需的有限活動中繼資料，例如雜湊化的 `project_root` scope key、host / model 名稱、時間戳、聚合語言統計和檔案增量計數；預設不會送出原始本地路徑、原始碼正文、raw prompt 或 raw transcript。

## 你會得到什麼

- 一個可直接部署的 FastAPI 執行面，位於 `apps/api`，並內建來自 `apps/web` 的 dashboard。
- 放在 `packages/collector-core` 的共享採集、緩衝和投遞邏輯。
- 穩定適配 `Claude Code` 與 `Codex`。
- 實驗適配 `Gemini CLI` 與 `OpenCode`。
...
- 可以直接用來排查的診斷入口：`/healthz`、`/api/v1/status`、`doctor`、`pending`

## 用 Coding Agent 一鍵安裝

如果你想走最快路徑，可以直接在倉庫目錄裡打開 `Claude Code`、`Codex` 或 `OpenCode`，然後把下面這段提示詞完整貼給 agent。

- 先讓 agent 閱讀目前這份 README 和 `docs/self-hosting-and-integration.md`。
- 在你的機器上逐條確認它要執行的命令，尤其是環境變數寫入和長時間執行的服務命令。
- 如果你想手動安裝，可以直接跳到下面的「快速開始」。

```text
你現在位於 Clipulse 倉庫根目錄。先閱讀 README.zh-TW.md 和 docs/self-hosting-and-integration.md，然後幫我在這台機器上完成一次完整的本地安裝。

目標：
1. 安裝所需的 Node.js 與 Python 依賴。
2. 為受保護的本地部署設定環境變數。如果我還沒有提供真實密鑰，就先用明顯的占位值，並在最後清楚告訴我哪些值必須替換。
3. 執行資料庫遷移。
4. 在 127.0.0.1:8000 啟動 API。
5. 透過本地 API 送出倉庫內建的 Codex smoke fixture。
6. 確認 dashboard 登入頁可以打開，並告訴我如何登入。
7. 如果某一步失敗，先排查並修復，再繼續，直到本地安裝可用。
8. 不要建立 tag、release，也不要修改無關檔案。
...
3. 透過穩定的 `Codex` adapter 路徑送出一條倉庫內建 smoke fixture。
...
當你設定了 `CLIPULSE_ENABLE_PUBLIC_READS=1` 和 `CLIPULSE_PUBLIC_BASE_URL` 之後，`/api/v1/public/readme/top-language` 會回傳一段可以直接貼到其他專案 README 的 markdown：
...
- [自託管與接入指南](./docs/self-hosting-and-integration.md)：部署模式、鑑權、反向代理、探針與 adapter 接線
- [架構總覽](./docs/architecture.md)：資料流、信任邊界和執行面
...
- [Contributing](./CONTRIBUTING.md)：貢獻約定和公開文件路由規則
- [Support](./SUPPORT.md)：公開求助路徑以及提問時該帶什麼資訊
- [Security policy](./SECURITY.md)：漏洞與隱私問題的私密回報方式
- [Changelog](./CHANGELOG.md)：面向發布的變更記錄

<details>
<summary>Adapter 入口與示例配置</summary>
...
- 穩定示例配置：[packages/adapter-claude/hooks/hooks.json](./packages/adapter-claude/hooks/hooks.json)、[packages/adapter-codex/examples/hooks.json](./packages/adapter-codex/examples/hooks.json)
...
- 實驗示例配置：[packages/adapter-gemini/examples/.gemini/settings.json](./packages/adapter-gemini/examples/.gemini/settings.json)、[packages/adapter-opencode/examples/clipulse.ts](./packages/adapter-opencode/examples/clipulse.ts)
...
- 構建後的 Python artifact 說明見 [docs/release-and-packaging.md](./docs/release-and-packaging.md) 和 [README.package.md](./README.package.md)。它們會打包 API runtime、dashboard 資源和 `/contracts/*`。
- `npm run check:release:prep` 是穩定面的 release-ready 預檢，`npm run check:release:prep:full` 會把實驗 adapter 一起納入。
- 如果只想公開只讀能力，發布 `/api/v1/badges/*` 和 `/api/v1/public/readme/*`，然後設定 `CLIPULSE_ENABLE_PUBLIC_READS=1` 與 `CLIPULSE_PUBLIC_BASE_URL`。
- 只有當 public outlet 位於獨立 origin 或代理路徑，才額外設定 `CLIPULSE_PUBLIC_PROBE_URL`，讓 `npm run smoke:deployment` 直接探測它。
...
- 漏洞、隱私洩露和任何需要私密處理的報告，走 [SECURITY.md](./SECURITY.md)。
