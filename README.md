...
- `npm run check:release:prep` 是稳定面的 release-ready 预检，`npm run bundle:stable` 只刷新带版本号的稳定 adapter bundle；完整 release 资产还需要重新生成 manifest / checksum 并运行 `npm run check:release-assets:stable`。`npm run check:release:prep:full` 会把实验 adapter 一起纳入。
...
- 如果你想验证 public 负路径，可以用 `CLIPULSE_EXPECT_PUBLIC_READS_MODE=disabled` 或 `CLIPULSE_EXPECT_PUBLIC_READS_MODE=misconfigured` 运行 `npm run smoke:deployment`。`misconfigured` 用来探测“public outlet 只部分可用”的场景，例如 badge 路由仍可读，但 README snippet 路由在代理或出口配置上返回 `503`；它不是一个建议保留的 Clipulse 应用配置。
