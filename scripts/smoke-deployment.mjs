...
export function parseDeploymentSmokeEnv(env = process.env) {
  ...
  return {
    baseUrl,
    serverToken: (env.CLIPULSE_SERVER_TOKEN ?? '').trim() || null,
...
export async function runDeploymentSmoke({
  ...
  const authorization = serverToken ? `Bearer ${serverToken}` : null
  ...
  if (!serverToken) {
    ...
    if (expectPublicReads) {
      const badgeResponse = await fetchImpl(`${baseUrl}/api/v1/badges/top-language.svg`)
      await assertResponseOk(badgeResponse, 'public badge probe failed')
      const readmeResponse = await fetchImpl(`${baseUrl}/api/v1/public/readme/top-language`)
