...
- より詳しい adapter 境界は `packages/adapter-claude/README.md` を参照してください。checked-in の canonical wiring source は `packages/adapter-claude/hooks/hooks.json` で、ホストが `PostToolUseFailure` / `StopFailure` / `SessionEnd` / `PreCompact` を出せるなら、それらは有意味な cleanup / wait 境界として配線を維持するのが推奨です。一方で `SubagentStop` は transcript-state cleanup boundary ではありません。公開 file-delta contract も引き続き patch-backed transcript change だけを約束します。
...
- `packages/adapter-opencode/dist/plugin.js` は依然として薄い bridge 入口であり、そのまま使う完全な plugin module ではありません。OpenCode の source of truth は `packages/adapter-opencode/README.md` と checked-in wrapper 例 `packages/adapter-opencode/examples/clipulse.ts` に意図的に集約し、トップレベル docs では第二の wrapper-contract copy を維持しません。
- `packages/adapter-opencode` は引き続き明示的な `file.edited` を既定の高信頼 delta 入口として扱い、`session.diff` は `CLIPULSE_OPENCODE_ENABLE_SESSION_DIFF=1` を明示的に設定した場合にだけ有効な default-off のままです。その場合でも、checked-in wrapper 例が転送するのは最小の `{ path, additions, deletions }` だけです。
...
- ターミナル中心で確認したい場合は `node packages/collector-core/dist/cli.js doctor` または `pending` を使えます。ローカル operator surface は意図的にこの 2 つの read-only コマンドだけで、missing state dir も作成せずに確認します。ローカル state がまだ無い場合は “no local state directory yet” を明示し、`doctor` は mixed backlog、quarantine-only、orphan-only、そして `stale_backlog` / `spool_size_cap` の retention ヒントも出します。未知コマンドは明示的に `doctor` へ fallback します。
